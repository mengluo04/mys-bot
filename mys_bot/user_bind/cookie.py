...
from .data_source import get_user_bind, set_user_bind


async def get_user_cookie(
        bot_id: str, user_id: str, uid: Optional[str], game: str
) -> Optional[str]:
...
async def get_user_cookie_with_fp(bot_id: str, user_id: str, sr_uid: Optional[str], game: str):
    select_user = await get_user_bind(bot_id, user_id, game)
    select_uid = [user.uid for user in select_user]
    if not select_uid:
...
async def get_user_stoken(
        bot_id: str, user_id: str, sr_uid: Optional[str], game: str
) -> Optional[str]:
    select_user = await get_user_bind(bot_id, user_id, game)
    select_uid = [user.uid for user in select_user]
    if not select_uid:
...
async def get_public_cookie(bot_id: str) -> Optional[str]:
    select_user = await get_user_bind(bot_id, "0")
    if not select_user:
        return None
...
async def set_cookie_expire(bot_id: str, user_id: str, sr_uid: str, game: str):
    select_user = await get_user_bind(bot_id, user_id, game)
    select_uid = [user.uid for user in select_user]
    if sr_uid in select_uid:
        user = select_user[select_uid.index(sr_uid)]
